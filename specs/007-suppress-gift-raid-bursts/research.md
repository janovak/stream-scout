...
use. Placement therefore co-locates on an existing connection first, grows a
new connection while below the three-connection maximum, and only at that
maximum or after growth fails may atomically take one slot on each of two
connections (D14a, R5). The remaining exposures — enabled
...
arithmetic, and are why exact capacity may be reached only after E2a, the
account sweep, and accumulation or safe seeding of 450 valid incumbents.
...
| `route()` (`:730`) | first connection with `load < cap` | must place **two** subscriptions in this order: co-locate on an existing connection, grow while fewer than three connections exist, then — only at the maximum or after growth fails — place one slot on each of two connections when the pool has at least two usable slots (D14a) |
...
`docker-compose.yml` carries the Feature 007 safe default
`JOIN_THRESHOLD=400` / `LEAVE_THRESHOLD=${LEAVE_THRESHOLD:-400}`, itself
reduced from the pre-007 `800` / `900`. FR-013 sets the final
operator-selected target: entry `400`, retention and maximum `450`.
...
Consequently, changing only `LEAVE_THRESHOLD` from 400 to 450 does **not**
create 50 channels. Starting from a 400-channel set, the retained band fills
through ranking turnover: new top-400 channels enter while displaced
incumbents remain at ranks 401-450. An exact-450 validation therefore waits for
450 valid incumbents to accumulate or uses an explicit safe validation seed.
At every smaller desired count, the relational convergence checks still apply.
...
`full_at` behaviour are proved deterministically before an operator selects retention
450, and deployed convergence is proved at
...
| **D10a** | Keep the checked-in safe compose default at **400/400** and use approved **entry 400 / retention-and-maximum 450** as the operator-selected final target after E1/E2a and the account sweep. `compute_desired_set()`'s existing `(previous \| top_join) & top_leave` produces the required asymmetry without admitting fresh rank-401-450 channels, so no hysteresis logic is added. `desired_set_churn_total` is retained as advisory bounded-label telemetry with no numeric gate (§6, NFR-007, SC-011). | Checking in a live 450 literal; silently deviating from the selected target; special-casing hysteresis in code; keeping a numeric churn release gate; or using equal 450/450 thresholds. |
...
| **D12** | Deployment is staged, and no stage takes two risks at once (autonomous decisions 16 and 27). (1) The ramp is reduced to 400/400 on the **current** single-subscription revision and allowed to converge — 400 × 1 = 400 of 900, unconditionally capacity-safe and **not** gated by E1. (2) The feature revision is deployed with the checked-in safe default retention **400**, so first dual convergence is 400 channels / 800 subscriptions with a cushion; E1 and E2a are taken there. (3) The account is swept for enabled subscriptions the pool does not own. (4) Only then may the operator select `LEAVE_THRESHOLD=450`. That setting admits no fresh rank-401-450 channels; growth above 400 occurs through ranking turnover or an explicit safe validation seed. E2b relations apply at every desired count ≤450, while exact 450/900 checks wait until 450 valid incumbents have accumulated or been seeded. Rollback runs the same logic in reverse: gating off; on a capacity incident lower retention to 400 and reconverge; unwind the transport **while thresholds are 400/400**; wait for the notification subscriptions to disappear; only then restore the single-subscription ramp. | One-step deploy of thresholds and transport together (the Redis-resident desired set makes the reconciler chase far more subscriptions than the ceiling allows before the first poll rewrites it — R9). Deploying the dual transport straight to the operator-selected target (first dual convergence and first exact-capacity operation in one step, with no cushion to diagnose from). Raising the retention threshold before unwinding the auxiliary subscriptions during rollback (R11a). |
...
| **R2** | Rendezvous imbalance puts one connection at 300 while the pair for a channel needs two slots. | `route()` first co-locates on an existing connection, then grows while fewer than three connections exist. Only at the maximum or after growth fails does the pair split across two connections, with both slots tracked independently and reserved atomically (D14a). | Slightly less locality; a socket death then touches two channels' partial coverage rather than one channel's whole coverage. Both states are already modelled. |
...
| **R5** | **Slot fragmentation at exact capacity.** With 900 of 900 slots spent, parity across three connections can leave the last two free slots on two *different* connections. A placement that only takes whole pairs on one connection reports a pool with room as full and stalls convergence at 449 channels. | D14a's atomic split-pair fallback runs only after co-location is unavailable and growth is impossible at the three-connection maximum or has failed: when pool-wide usable free slots ≥ 2, reserve one on each of two connections in a single critical section. Deterministic tests construct the fragmented state directly; E2b exercises it deployed when exact-450 validation is available. | Reduced locality for the split channels, and a socket death leaves them partially covered — a state the reconciler already repairs. Repeated split/repair cycles can fragment further; that is visible through coverage state and connection occupancy. |
...
| **R15** | **Foreign enabled subscriptions.** Anything enabled on the client-id/user-id pair that this pool does not own — an earlier revision's leftovers, another process, a manual experiment — consumes the same 900 slots. With no reserve, one such subscription makes the 450th channel unplaceable, and the symptom is indistinguishable from a defect in the pool. | An account-wide enumeration and sweep is a required rollout step **before** the operator selects retention 450 (plan forward order, D12), and the capacity classification on the create-failure metric plus per-connection occupancy makes the resulting shortfall legible rather than mysterious. | Nothing prevents a new foreign subscription appearing later; it is detected as a gap between complete-coverage channels × 2 and total subscriptions, and by capacity refusals at a channel count below 450. |
...
| **R18** | **Exact convergence is slower and less forgiving.** At 900 of 900 there is no slack to absorb in-flight create/delete overlap, so a churn event at the boundary can transiently need a slot that a pending delete still holds. | Convergence is proved deterministically before the operator selects 450 (T060-T062) and deployed at 400/800 first (E2a); E2b relations apply at every desired count, while exact capacity waits for 450 accumulated or safely seeded incumbents. A hard capacity condition does **not** arm a transient growth backoff, so the next pass retries immediately once the delete lands. | A boundary channel may take an extra reconcile pass to become complete at exact capacity. Visible as a short-lived partial-coverage series, not as data loss. |
...
| **E2b** | After the account sweep and operator selection of entry 400 / retention 450, relational convergence applies at every desired count ≤450: complete coverage **== desired**; total subscriptions **== 2 ×** complete coverage; every connection occupancy **≤ 300**; summed occupancy **==** total subscriptions; and summed usable free slots **== 900 −** total subscriptions. Exact-450 validation is conditional on accumulating 450 valid incumbents through ranking turnover or supplying an explicit safe validation seed; then require subscriptions **== 900**, free slots **== 0**, and exclusion of the 451st qualifying channel. In that condition exercise grow-before-split placement, split-fragment replacement, capacity classification without transient backoff, and below-cap `full_at` re-evaluation. | Blocks the amended SC-006 sign-off and enabling gating (FR-013, FR-014, NFR-001, decisions 27-28). Approximate readings are never acceptable, and lack of 450 valid incumbents does not waive the always-applicable relational checks |
