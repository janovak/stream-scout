...
the timestamp assigner has independently protected event time using the
same injectable/current wall-clock basis: parsed occurrence time through
`source_clock_ms + 30_000` is assigned directly, while missing, unreadable, or
over-bound occurrence time uses Kafka `record_timestamp` without rewriting the
payload. That assigner is reachable only through
`assign_timestamps_and_watermarks()` on the stream returned by `from_source`
(I22); handed to `from_source` itself it is silently discarded and event time
becomes the Kafka record timestamp for every record. After schema and field
validation, and before delivery observation or
state access, `process_element2` enforces the same fixed contract bound
...
| **I15** | The suppression input never becomes the binding watermark minimum in steady state: its idleness timeout is strictly less than the chat stream's, and its partition count equals `FLINK_PARALLELISM` (research R3). Because watermarks and idleness are emitted by the post-source assignment operator rather than by the Kafka source (I22), the partition-count equality must extend to the assignment operator's parallelism for this to hold |
