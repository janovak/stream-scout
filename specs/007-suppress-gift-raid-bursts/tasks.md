...
> **Historical pre-amendment phase.** The checked work below established the
> original 400/400 model. Decisions 27-28 and T059-T068 supersede its capacity
> ceiling, reserve, and boundary assertions while preserving its lifecycle work.
...
- [x] T027 [US3] **Historical; superseded by T060/T064.** After T026 passes, add failing static/runtime tests for checked-in 400/400 thresholds, a checked-in `AUXILIARY_REFUSAL_RETRY_SECONDS=3600`, a 400-entry maximum desired set, the accepted zero-width band behavior, and `desired_set_churn_total` incrementing by entered plus departed channels with no per-channel label growth so the then-current NFR-007 bound of 8 changes per poll is directly computable in `services\stream-monitoring\test_stream_monitoring.py`
- [x] T028 [US3] **Historical; superseded by T063/T064.** After T026 and T027, set `JOIN_THRESHOLD=400`, `LEAVE_THRESHOLD=400`, and `AUXILIARY_REFUSAL_RETRY_SECONDS=3600` with the 400-channel/800-subscription/100-headroom explanation in `docker-compose.yml`, and implement bounded `desired_set_churn_total` accounting in `services\stream-monitoring\stream_monitoring_service.py`, recording the then-current NFR-007/SC-011 24-hour churn evidence requirement
...
- [x] T053 [P] Document the original 400-channel capacity model, two coverage types, coverage/subscription units, the bounded auxiliary-refusal hold-off and its expiry/reconnect recovery, the suppression topic and windows, every new suppression metric and log, the Prometheus windowed reading of healthy/lagging/idle-unknown delivery, fail-open diagnosis, the operator kill switch, and an explicitly pending operator-run E1-E5 checklist in `OPERATIONS.md`; its former numeric churn gate and original capacity/rollback wording are superseded by T064-T065
...
- [x] T061 [US3] Add failing exact-capacity behaviour tests for slot fragmentation and legibility: placement co-locates on an existing connection, grows while fewer than three connections exist, and only at the maximum or after growth fails places a pair as one slot on each of two connections when the pool has at least two usable slots; cover atomic all-or-nothing reservation of that pair under concurrency, retention of the successful half after a post-reservation failure, replacement of a lost split fragment converging back to complete without exceeding 900, reconnect and adoption consuming no additional slot, a hard capacity exhaustion raising the distinct capacity error with its own metric classification rather than a provider refusal or transient fault and without arming the transient growth backoff or writing the durable refusal cache, and `full_at` being cleared and re-evaluated on reconnect and retirement with below-cap full state exposed, in `services\stream-monitoring\test_stream_monitoring.py`
- [x] T062 [US3] After T060 and T061 fail for the intended reasons, implement co-location-first placement, growth while below the three-connection maximum, and the atomic two-connection split-pair reservation fallback only at the maximum or after growth fails; add the distinct `PoolCapacityError` and its capacity metric classification, suppress transient growth backoff for a hard capacity condition, and clear, re-evaluate, and expose below-cap `full_at`, keeping `reconciler.py` channel-keyed and unchanged, in `services\stream-monitoring\eventsub_pool.py`
- [x] T063 [US3] After T062 passes, set the checked-in safe default `JOIN_THRESHOLD=400` and `LEAVE_THRESHOLD=${LEAVE_THRESHOLD:-400}` with a comment stating that 400 is the initial dual-coverage value and 450 is the operator-selected final retention target after E1/E2a and the account sweep; align the runtime capacity constants and comments with the 450 × 2 = 900 exact ceiling and absence of a guaranteed reserve in `services\stream-monitoring\eventsub_pool.py` and `services\stream-monitoring\stream_monitoring_service.py`
- [x] T064 Keep `desired_set_churn_total` as advisory bounded-label telemetry and remove every numeric release gate attached to it — the 2%/8-changes-per-poll bound, the 24-hour observation window, and any assertion or comment treating it as blocking — in `services\stream-monitoring\stream_monitoring_service.py` and `services\stream-monitoring\test_stream_monitoring.py`
- [x] T065 Update the operator runbook for the amended capacity model: checked-in safe default 400 and operator-selected final retention target 450, exact 900-of-900 occupancy with no reserve, literal `eventsub_connection_full`, `eventsub_connection_free_slots`, `eventsub_connection_full_below_cap`, and `subscription_create_failures_total{reason="capacity"}` signals, the advisory churn reading with no release gate, the staged rollout (400/400 single-subscription convergence, dual transport at the safe 400 default with gating off, E1 and E2a, account-wide foreign-subscription sweep, operator selection of 450, relational E2b checks at every desired ≤450, and conditional exact-450 validation after 450 valid incumbents accumulate through ranking turnover or an explicit safe seed, then E3, E4, E5), and the capacity-safe rollback that lowers retention to 400 and reconverges before the transport is unwound, in `OPERATIONS.md`
...
- T060 and T061 precede T062, and T062 precedes T063 — the amended capacity
  ...
  and the pool change lands before the safe-default configuration and before
  any operator selects retention 450. This is the same ordering rule that made
  T026/T027 precede T028, and it must not be relaxed.
...
| SC-008 | T016-T017, T024-T025, T033-T035, T039-T043, T053 → **amended**: T061-T063, T065, T067 |
...
| T028 | Checked-in `JOIN_THRESHOLD=400` / `LEAVE_THRESHOLD=400` and bounded churn accounting | T063 (checked-in safe 400 default plus operator-selected final 450 target) and T064 |
...
| E2b | After the account-wide foreign-subscription sweep and operator selection of entry 400 / retention 450, relational convergence holds at every desired <=450: complete == desired, subscriptions == 2 x complete, every connection <=300, summed occupancy == subscriptions, and usable free slots == 900 - subscriptions. Exact-450 validation is conditional on accumulating 450 valid incumbents through ranking turnover or an explicit safe seed; then require 900 subscriptions, zero free slots, and the 451st exclusion. The grow-before-split, fragment-replacement, capacity-classification, and below-cap `full_at` drills all pass (SC-006, decisions 27-28). |
...
- This ledger was fixed at **58 tasks, T001-T058** for the original scope, and
  ...
  split, or renumber them. In the amendment phase T059-T066 are complete;
  T067-T068 remain open.
- A checked box records the work that was completed against the task text as it
  ...
  their existing marks are preserved; the assignment-path work carried by T058
  is now complete. Decisions 27-28 change what
...
- `docker-compose.yml` checks in `SUPPRESSION_GATING_ENABLED=false` while the
  ...
  an operator action taken only after E1, E2a, E2b, and E3 pass. Churn remains
  advisory and imposes no observation-window gate.
