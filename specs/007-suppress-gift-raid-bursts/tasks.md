...
- [x] T041 [US1] Implement defensive version-1 decoding, payload field validation, trigger allow-list enforcement, and `occurred_at_ms` timestamp assignment following `specs\007-suppress-gift-raid-bursts\contracts\suppression-events.schema.md` in `services\flink-job\clip_detector_job.py`; after schema/field decode and before delivery observation or state access, reject `occurred_at_ms > consumer_receipt_ms + SUPPRESSION_MAX_FUTURE_SKEW_SECONDS * 1000` as malformed fields; derive routing and state from the payload `broadcaster_id` and implement no Kafka-key comparison, because the value-only deserializer never surfaces the key to the operator
...
- [x] T045 [US1] Build the second Kafka source from `SuppressionSourceSettings` with latest offsets and the suppression watermark/idleness strategy, key both inputs on the payload `broadcaster_id`, and connect them into `AnomalyDetector` without changing `CommandFilter` or `ClipCreator` in `services\flink-job\clip_detector_job.py`
...
| E4 | Real gift/raid capture confirms trusted-record `suppression_delivery_age_seconds` against the 30-second warning threshold, malformed-future rejection visibility, real-burst notice-bounded suppression for SC-003, window-default tuning/adequacy for SC-005, trigger mapping, and unaffected out-of-window/pre-notice clips. |
